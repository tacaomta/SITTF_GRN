...
class GeneNetwork:
    ...
    def compareTo(self, other: object, start=0) -> float:
        matrix = np.zeros((self.timestepsNumber, len(self.nodes)))
        differences = 0
        for i in range(len(self.nodes)):
            for j in range(start, self.timestepsNumber):
                if self.nodes[i].values[j]!=other.nodes[i].values[j]:
                    differences+=1
                    matrix[j, i] = 1
        return differences/(len(self.nodes)*(self.timestepsNumber - start)), matrix
